defmodule Struct.Integration.BuildTest do
  use Struct.TestCase
  ...
  test "pass `empty_values` to use/2" do
    module = create_struct [empty_values: ["empty_val"]] do
      field :a
    end
  ...
  test "pass `make_map` to use/2" do
    module = create_struct [make_map: true] do
      field :a
    end
  ...
  test "make nested fields" do
    ...
    assert Struct.Integration.BuildTest_66 == root.__struct__
    assert Struct.Integration.BuildTest_66.Parent == parent.__struct__
    assert Struct.Integration.BuildTest_66.Parent.NestedStruct == nested.__struct__
    assert Struct.Integration.BuildTest_66.Parent.NestedStruct.SOME_OF1 == some.__struct__
  end

  test "raise when try to use non-atom field name" do
    assert_raise(Struct.DefinitionError, ~s(expected atom for field name, got `"key"`), fn ->
      create_struct do
        field "key", :string
      end
    end)
  ...
  test "raise when trying to use function as default argument" do
    assert_raise(Struct.DefinitionError, ~s(default value cannot to be a function), fn ->
      defmodule M do
        use Struct
  ...
  test "raise when trying to use undefined module as custom type" do
    assert_raise(Struct.DefinitionError, ~s(undefined module Elixir.UndefinedModule), fn ->
      create_struct do
    ...
    assert_raise(Struct.DefinitionError, ~s(undefined complex type :any), fn ->
      create_struct do
  ...
  test "raise when trying to use custom type that doesn't have cast/1 function" do
    assert_raise(Struct.DefinitionError, ~s(undefined function cast/1 for Elixir.CustomTypeEmpty), fn ->
      create_struct do
  ...
  test "raise when trying to include invalid struct (some module)" do
    assert_raise(Struct.DefinitionError, ~s(provided Elixir.CustomTypeEmpty is not Struct module), fn ->
      create_struct do
